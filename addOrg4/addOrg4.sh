...
function generateOrg3() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create Kmadmin Identities ######################"
    echo "##########################################################"
  ...
  echo
  echo "Generate CCP files for Kmadmin"
  ./ccp-generate.sh
}
...
function generateOrg3Definition() {
  ...
  echo "##########################################################"
  echo "#######  Generating Kmadmin organization definition #########"
  echo "##########################################################"
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg KmadminMSP > ../organizations/peerOrganizations/kmadmin.com/org3.json
   res=$?
   { set +x; } 2>/dev/null
   if [ $res -ne 0 ]; then
     echo "Failed to generate Kmadmin config material..."
     exit 1
   fi
  echo
}
...
function Org3Up () {
  ...
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to start Kmadmin network"
    exit 1
  fi
}
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/kmadmin.com" ]; then
    generateOrg3
  ...
  # Kmadmin to the network
  echo
  echo "###############################################################"
  echo "####### Generate and submit config tx to add Kmadmin #############"
  echo "###############################################################"
  docker exec Kmadmincli ./scripts/org4-scripts/step1org4.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
  ...
  echo
  echo "###############################################################"
  echo "############### Have Kmadmin peers join network ##################"
  echo "###############################################################"
  docker exec Kmadmincli ./scripts/org4-scripts/step2org4.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to have Kmadmin peers join network"
    exit 1
  fi
...
if [ "$MODE" == "up" ]; then
  echo "Add Kmadmin to channel '${CHANNEL_NAME}' with '${CLI_TIMEOUT}' seconds and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE}'"
  echo
elif [ "$MODE" == "down" ]; then
  EXPMODE="Stopping network"
elif [ "$MODE" == "generate" ]; then
  EXPMODE="Generating certs and organization definition for Kmadmin"
else
  printHelp
  exit 1
fi
