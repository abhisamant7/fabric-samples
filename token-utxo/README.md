...
While a transfer transaction spends UTXOs and creates new UTXOs for the recipient(s), a mint transaction can create new UTXOs. In this sample it is assumed that only one organization (played by Dfarmadmin) is in a central banker role and can mint new tokens owned by their client ID. Any client from any organization can transfer tokens in a UTXO transaction.

In this tutorial, you will mint and transfer tokens as follows:

- A member of Dfarmadmin uses the `Mint` function to create a UTXO representing a number of tokens. The `Mint` function reads the certificate information of the client identity that submitted the transaction using the `GetClientIdentity.GetID()` API and assigns the UTXO ownership to the minter client ID.
...
The test network is deployed with two peer organizations. The `createChannel` flag deploys the network with a single channel named `mychannel` with Dfarmadmin and Yngadmin as channel members.
...
The smart contract supports UTXO ownership based on individual client identities from organizations that are members of the channel. In our scenario, the minter of the tokens will be a member of Dfarmadmin, while the recipient will belong to Yngadmin. To highlight the connection between the `GetClientIdentity().GetID()` API and the information within a user's certificate, we will register two new identities using the Dfarmadmin and Yngadmin Certificate Authorities (CA's), and then use the CA's to generate each identity's certificate and private key.
...
The terminal we have been using will represent Dfarmadmin. We will use the Dfarmadmin CA to create the minter identity. Set the Fabric CA client home to the MSP of the Dfarmadmin CA admin (this identity was generated by the test network script):
```
...
```
fabric-ca-client enroll -u https://minter:minterpw@localhost:7054 --caname ca-org1 -M ${PWD}/organizations/peerOrganizations/dfarmadmin.com/users/minter@dfarmadmin.com/msp --tls.certfiles ${PWD}/organizations/fabric-ca/org1/tls-cert.pem
```

Run the command below to copy the Node OU configuration file into the minter identity MSP folder.
```
cp ${PWD}/organizations/peerOrganizations/dfarmadmin.com/msp/config.yaml ${PWD}/organizations/peerOrganizations/dfarmadmin.com/users/minter@dfarmadmin.com/msp/config.yaml
```

Open a new terminal to represent Yngadmin and navigate to fabric-samples/test-network. We'll use the Yngadmin CA to create the Yngadmin recipient identity. Set the Fabric CA client home to the MSP of the Yngadmin CA admin:
```
cd fabric-samples/test-network
export PATH=${PWD}/../bin:${PWD}:$PATH
...
```
fabric-ca-client enroll -u https://recipient:recipientpw@localhost:8054 --caname ca-org2 -M ${PWD}/organizations/peerOrganizations/yngadmin.com/users/recipient@yngadmin.com/msp --tls.certfiles ${PWD}/organizations/fabric-ca/org2/tls-cert.pem
```

Run the command below to copy the Node OU configuration file into the recipient identity MSP folder.
```
cp ${PWD}/organizations/peerOrganizations/yngadmin.com/msp/config.yaml ${PWD}/organizations/peerOrganizations/yngadmin.com/users/recipient@yngadmin.com/msp/config.yaml
```

## Mint some tokens

Now that we have created the identity of the minter, we can invoke the smart contract to mint some tokens.
Shift back to the Dfarmadmin terminal, we'll set the following environment variables to operate the `peer` CLI as the minter identity from Dfarmadmin.
```
export CORE_PEER_TLS_ENABLED=true
export CORE_PEER_LOCALMSPID="DfarmadminMSP"
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/dfarmadmin.com/users/minter@dfarmadmin.com/msp
export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/dfarmadmin.com/peers/peer0.dfarmadmin.com/tls/ca.crt
export CORE_PEER_ADDRESS=localhost:7051
export TARGET_TLS_OPTIONS="-o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile ${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem --peerAddresses localhost:7051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/dfarmadmin.com/peers/peer0.dfarmadmin.com/tls/ca.crt --peerAddresses localhost:9051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/yngadmin.com/peers/peer0.yngadmin.com/tls/ca.crt"
```
...
The minter intends to transfer 100 tokens to the Yngadmin recipient, but first the Yngadmin recipient needs to provide their own client ID as the payment address.
...
Let's prepare the Yngadmin terminal by setting the environment variables for the Yngadmin recipient user.
```
export FABRIC_CFG_PATH=$PWD/../config/
export CORE_PEER_TLS_ENABLED=true
export CORE_PEER_LOCALMSPID="YngadminMSP"
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/yngadmin.com/users/recipient@yngadmin.com/msp
export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/yngadmin.com/peers/peer0.yngadmin.com/tls/ca.crt
export CORE_PEER_ADDRESS=localhost:9051
```

Using the Yngadmin terminal, the Yngadmin recipient user can retrieve their own client ID:
```
...
Let's base64 decode the client ID to make sure it is the Yngadmin recipient user:
```
...
The result shows that the subject and issuer is indeed the recipient user from Yngadmin:
```
x509::CN=recipient,OU=client,O=Hyperledger,ST=North Carolina,C=US::CN=ca.yngadmin.com,O=yngadmin.com,L=Hursley,ST=Hampshire,C=UK
```

After the Yngadmin recipient provides their client ID to the minter, the minter can initiate a transfer of tokens. We'll pass in the utxo_key of the UTXO with 5000 tokens to spend, and request that two new UTXOs get created, a UTXO with 100 tokens for the recipient, and a UTXO with 4900 tokens for the minter as the 'change'. Since the contract will create the UTXO output keys, we'll initially leave the output keys blank.
Back in the Dfarmadmin terminal, request the UTXO transfer. **Replace YOUR_UTXO_KEY below with the key you saved earlier**:
```
...
While still in the Dfarmadmin terminal, let's request the minter's UTXOs again:
```
peer chaincode query -C mychannel -n token_utxo -c '{"function":"ClientUTXOs","Args":[]}'
```
...
And then using the Yngadmin terminal, let's request the recipient's UTXOs:
```
peer chaincode query -C mychannel -n token_utxo -c '{"function":"ClientUTXOs","Args":[]}'
```
...
Congratulations, you've transferred 100 tokens! The Yngadmin recipient can now transfer tokens to other registered users in the same manner.
