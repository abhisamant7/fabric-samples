...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"KmadminMSP":.[1]}}}}}' config.json ./organizations/peerOrganizations/kmadmin.com/org4.json > modified_config.json
{ set +x; } 2>/dev/null
...
setGlobals 2
set -x
peer channel update -f org4_update_in_envelope.pb -c ${CHANNEL_NAME} -o orderer.dfarmorderer.com:7050 --ordererTLSHostnameOverride orderer.dfarmorderer.com --tls --cafile ${ORDERER_CA}
{ set +x; } 2>/dev/null
