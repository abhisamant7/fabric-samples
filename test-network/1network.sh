...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Create Dfarmadmin Identities"
    ...
    infoln "Create Yngadmin Identities"
  ...
  infoln "Generate CCP files for Dfarmadmin and Yngadmin"
...
## Bring up the network if it is not already up.
 ...
 scripts/1createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
  if [ $? -ne 0 ]; then
...
function deployCC() {
  ...
  scripts/1deployCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
